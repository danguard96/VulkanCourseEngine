...
VKAPI_ATTR void VKAPI_CALL vkDestroyDebugUtilsMessengerEXT(VkInstance instance,
    ...
    if (function != nullptr) function(instance, debug_messenger, allocator);
}
...
#pragma region VALIDATION_LAYERS
    ...
    ValidationCallback(VkDebugUtilsMessageSeverityFlagBitsEXT severity, VkDebugUtilsMessageTypeFlagsEXT type,
    ...
    std::vector<VkLayerProperties> Graphics::GetSupportedValidatedLayers() {
    ...
    bool Graphics::AreAllLayersSupported(gsl::span<gsl::czstring> layers) {
        std::vector<VkLayerProperties> supported_layers = GetSupportedValidatedLayers();

        return std::ranges::all_of(layers.begin(), layers.end(),
    ...
    void Graphics::SetupDebugMessenger() {
        ...
        if (result != VK_SUCCESS) spdlog::error("Cannot create debug messenger");
    }
...
#pragma region INSTANCE_AND_EXTENSIONS
    ...
    std::vector<gsl::czstring> Graphics::GetRequiredInstanceExtensions() {
        ...
        if (validation_enabled_) required_extensions.push_back(VK_EXT_DEBUG_UTILS_EXTENSION_NAME);

        if (!AreAllExtensionsSupported(suggested_extensions)) std::exit(EXIT_FAILURE);
...
#pragma region DEVICES_AND_QUEUES
    ...
    bool Graphics::AreAllDeviceExtensionsSupported(VkPhysicalDevice device) {
        ...
        return std::any_of(required_device_extensions_.begin(), required_device_extensions_.end(),
                           std::bind_front(IsExtensionSupported, available_extensions));
    }

    bool Graphics::IsDeviceSuitable(VkPhysicalDevice device) {
        QueueFamilyIndices families = FindQueueFamilies(device);
        return families.IsValid() && AreAllDeviceExtensionsSupported(device) && GetSwapchainProperties(device).
            IsValid();
    }
    ...
    void Graphics::PickPhysicalDevice() {
        ...
        std::erase_if(devices, std::not_fn(std::bind_front(IsDeviceSuitable, this)));
    ...
    void Graphics::CreateLogicalDeviceAndQueues() {
        ...
        std::set<std::uint32_t> unique_queue_families{
                picked_device_families.graphics_family.value(), picked_device_families.presentation_family.value()
            };
...
#pragma region PRESENTATION
    ...
    bool IsRgbaTypeFormat(const VkSurfaceFormatKHR& format_properties) {
        return format_properties.format == VK_FORMAT_R8G8B8A8_SRGB || format_properties.format ==
    ...
    VkSurfaceFormatKHR Graphics::ChooseSwapSurfaceFormat(gsl::span<VkSurfaceFormatKHR> formats) {
        if (formats.size() == 1 && formats[0].format == VK_FORMAT_UNDEFINED)
            return {
                    VkFormat::VK_FORMAT_R8G8B8A8_SNORM, VkColorSpaceKHR::VK_COLORSPACE_SRGB_NONLINEAR_KHR
                };
    ...
    VkExtent2D Graphics::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
        constexpr std::uint32_t kInvlidSize = std::numeric_limits<std::uint32_t>::max();

        if (capabilities.currentExtent.width != kInvlidSize) {
...
#pragma region GRAPHICS_PIPELINE
    ...
    void Graphics::CreateGraphicsPipeline() {
        std::vector<std::uint8_t> basic_vertex_data = ReadFile("./assets/shaders/basic.vert.spv");
        ...
        std::vector<std::uint8_t> basic_fragment_data = ReadFile("./assets/shaders/basic.frag.spv");
        ...
        if (vertex_shader == VK_NULL_HANDLE || fragment_shader == VK_NULL_HANDLE) std::exit(EXIT_FAILURE);
